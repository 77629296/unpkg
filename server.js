...
const privateNpmRegistryURL = 'http://10.213.3.212:4873'; //互联网npm地址

const publicNpmRegistryURL = 'http://registry.npmjs.org';
...
async function getPackage(packageName, version, log) {
  const tarballName = isScopedPackageName(packageName) ? packageName.split('/')[1] : packageName; // const tarballURL = `${npmRegistryURL}/${packageName}/-/${tarballName}-${version}.tgz`;
  // 这里会被切割@，外网不会
  // 获取正确的包的url
  ...
  if (isPrivatePage) {
    tarballURL = `${npmRegistryURLPrivate}/${packageName}/-/${tarballName}-${version}.tgz`;
  } else {
